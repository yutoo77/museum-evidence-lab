...
| FAQの明示承認と資料更新時の失効 | 古い回答の再利用を抑える。対象者別の説明方式では未対応のため使わない |
...
4. 「説明案」と「この説明の根拠」を照合する。「原文から回答」に切り替えて比較できる。

初回クローンに登録済み資料・索引・FAQは含みません。以後は`start_lab.cmd`で起動、`stop_lab.cmd`で停止します。ブラウザを閉じるだけでは停止しません。
